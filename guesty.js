...
const LISTINGS = {
  ...
  'regent-sol':     { id: '6ab471bc946a45001142aa79', name: 'Regent Sol',                          basePrice: 278, cleaningFee: 0,   minNights: 1, accommodates: 8,  bedrooms: 2, bathrooms: 2.5, city: 'Austin', state: 'Texas' },
};
...
async function getListings() {
  ...
  const out = Object.entries(LISTINGS).map(([slug, meta]) => {
    ...
    return {
      slug,
      id: meta.id,
      // Display name: OUR brand name, not Guesty's listing title — properties
      // were renamed on the site (e.g. Regent Lakefront/Waterfront) while the
      // Guesty/Airbnb listings still carry the old names.
      title: meta.name,
      nickname: l.nickname,
      city: l.address?.city,
